...
const ViewTaskAdmin = ({staffId, specificStaff, close, open}: any) => {
  ...
  return (
    <div>
        ...
        <div className='min-h-screen fixed right-0 top-0 bottom-0 shadow-transparent bg-[#8996d7] px-5 py-10 z-50 min-w-[300px] md:min-w-[450px] animate-slide-in-right translate-x-[6px] before:translate-x-[-66px] duration-150 transition-transform after:translate-x-[66px]'>
        <span className='cursor w-full flex justify-end cursor-pointer group' onClick={close}>
           ...
           <p className='text-[20px] font-bold'>Staff Tasks</p>
           <ul className='flex flex-col gap-[50px] mt-[20px]'>
             ...
             <button onClick={() => open(specificStaff._id, specificStaff.fullName, specificStaff.email)} className='text-center w-full mt-[30px] mx-auto py-[21px] border border-[#000] '>Add More Tasks</button> 
          </div>
        </>}
    </div>
  )
