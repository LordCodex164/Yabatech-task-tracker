...
const DropDownComponent = ({options, selectedValue, setSelectedValue, selected,setSelected}: DropDownProps) => {
    ...
    return (
        <>
         <div onClick={handleSelect} className='relative bg-[#fff] cursor-pointer flex flex-row border-[1px] px-[10px] py-[25px] mb-[26px] w-full min-w-[97px] max-h-[50px] border-[#1F2937] items-center text-[16px]'>
                   {selectedValue}
                   <RxCaretDown/>
                  {selected && (
