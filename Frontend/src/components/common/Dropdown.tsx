...
const DropDownComponent = ({options, selectedValue, setSelectedValue, selected,setSelected}: DropDownProps) => {
    ...
    return (
        <>
         <div onClick={handleSelect} className='relative bg-[#fff] cursor-pointer flex flex-row border-[1px] px-[10px] w-full min-w-[97px] max-h-[50px] border-[#1F2937] items-center text-[16px]'>
                   {selectedValue}
                   <RxCaretDown/>
                  {selected && (
                    <div className='absolute top-[25px] left-0  w-full min-w-[97px] min-h-full p-1 text-sm black-text-3 bg-white shadow-[1px_4px_12px_-1px_rgba(44,78,39,0.15)] rounded'>
